...
public class TT2Decrypt {

    private Integer CF = 0;


    public TT2Decrypt(long[] content) {
        this.content = Arrays.copyOfRange(content, 38, content.length);
        this.list_9C8 = Arrays.copyOfRange(content, 6, 38);
    ...
    public void check_log(long[] temp_list) {
        int length = temp_list.length;
        String[] tmp = new String[length];
        for (int index = 0; index < length; index++) {
            long item = temp_list[index];
            String s = Long.toHexString(item);
            tmp[index] = s;
        }
        //System.out.println("长度"+ Integer.toHexString(tmp.length));
        //System.out.println("内容"+ JSON.toJSONString(tmp));
    }
    ...
    public long ADCS(long A, long B) {
        ...
        //System.out.println("ADCS CF current Value :" + CF);
        return D;
    }
    ...
    public long LSRS(long num, int k) {
        ...
        //System.out.println("LSRS CF current Value :" + CF);
    ...
    public long[] calculate(long[] content) {
        ...
        // check_log(tmp_list)
        //System.out.println("****");
        //System.out.println(JSON.toJSONString( tmp_list ));
        ...
        for (int i = 0; i < tmpListSize / 0x80; i++) {
            if (i == (tmpListSize / 0x80 - 1)) {
                ...
                //check_log(convertLongArray( tmp_list));
            }
    ...
    public String decrypt() throws Exception {
        ...
        long[] keyLongs = hex_list(list_378Array);
        long[] list_740Array = list_740.stream().mapToLong(Long::valueOf).toArray();

        SecretKeySpec skeySpec = new SecretKeySpec(changeLongArrayTobytes(keyLongs), "AES");
        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding");

        IvParameterSpec iv = new IvParameterSpec(changeLongArrayTobytes(list_740Array));
        cipher.init(Cipher.DECRYPT_MODE, skeySpec, iv);

        byte[] cipher_text = cipher.doFinal(changeLongArrayTobytes(this.content));
        long[] cipher_textLongs = changeToLongArray(cipher_text);
        long[] response = Arrays.copyOfRange(cipher_textLongs, 64, cipher_textLongs.length);

        String result = new String(GZIPUtil.uncompress(changeLongArrayTobytes(response)));

        return result;
    }

    public String decryptNoGz() throws Exception {
        long[] list_0B0 = ArrayUtils.addAll(calculate(this.list_9C8), ord_list);
        long[] list_5D8 = this.calculate(list_0B0);
        // System.out.println(JSON.toJSONString(list_5D8));
        //check_log(list_5D8)
        ...
        String result = new String(changeLongArrayTobytes(response));
    ...
    public static String decryptEn(byte[] bytes) {
        String result = "";
        try {
            long[] longs = changeToLongArray(bytes);
            result = new TT2Decrypt(longs).decrypt();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
    ...
    public static String decryptEnNoGz(byte[] bytes) {
        String result = "";
        try {
            long[] longs = changeToLongArray(bytes);
            result = new TT2Decrypt(longs).decryptNoGz();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
    ...
    public static void main(String[] args) throws Exception {
        byte[] bytes = "Need decrypt data".getBytes();
        long[] longs = changeToLongArray(bytes);
        String result = new TT2Decrypt(longs).decrypt();
